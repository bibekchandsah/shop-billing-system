...
export const DEFAULT_SETTINGS: AppSettings = {
  ...
  businessAddress: 'Putlisadak, Kathmandu, Nepal',
  businessContact: '',
...
export const saveAppSettings = async (userId: string, settings: AppSettings): Promise<void> => {
  ...
  try {
    const payload: Record<string, any> = {
      ...
      billTitle: settings.billTitle || DEFAULT_SETTINGS.billTitle,
      maxBillNumber: Number.isFinite(settings.maxBillNumber) && settings.maxBillNumber > 0 ? Math.floor(settings.maxBillNumber) : DEFAULT_SETTINGS.maxBillNumber,
      billNumberFormat: settings.billNumberFormat || 'numeric',
      billNumberPrefix: settings.billNumberPrefix || 'BILL-',
      numberSystem: settings.numberSystem || DEFAULT_SETTINGS.numberSystem,
      unitCategories: settings.unitCategories ?? DEFAULT_SETTINGS.unitCategories,
